...
/// ![Toggle rendering.](toggle/component_disabled.png)
///
...
public final class SparkUISwitch: UIControl {
    ...
    private let toggleHeight: CGFloat = Constants.Sizes.height
    private let toggleWidth: CGFloat = Constants.Sizes.width
    private let toggleSpacing: CGFloat = Constants.Sizes.padding
    ...
    @objc private func toggleLongGestureAction(_ sender: UILongPressGestureRecognizer) {
        ...
        case .began where !self.isReduceMotionEnabled:
            UIView.execute(animationType: self.viewModel.animationType) { [weak self] in
                ...
                self.toggleDotWidthConstraint?.constant = Constants.Sizes.dotIncreasePressedSize
    ...
    private func updateHover(show: Bool) {
        if show {
            let width = Constants.Sizes.hoverPadding
            let inset = -width / 2
